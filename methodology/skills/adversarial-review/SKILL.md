...
description: Fresh-context adversarial review of own-work artifacts — `--design`, `--acceptance`, plan, decomposition tree, bead set, cumulative diff. Use when about to stamp `verdict:pass` on own work, about to first-dispatch children in a multi-bead unit (review the *scoping*, not each item — boundaries, ordering, dependency edges, missing items), about to mark a unit complete, just drafted a `--design` / `--acceptance` / decomposition tree, or about to invoke `bd close` on a non-trivial bead. Triggers on "sanity-check this", "review my plan", "ready to ship the design", "before we start", "is this scoped right", "did I miss anything". Default-on for own work — never review your own contract, plan, or decomposition tree without fresh-context dispatch. One reviewer per round, orchestrator-judged convergence via countermand-test, three-state exit (pass / fail / raise). Distinct from review-v2 (heavy reference composition with mode switching and convergence-loop machinery).
---

## Purpose

Provide fresh-context adversarial rigor over any "own-work" artifact — a bead's `--design`, a proposed plan, a decomposition tree — so authors don't silently mark their own work converged. The fresh-context dispatch is the load-bearing element: agents inside the author frame inherit the author's blind spots; a separate fresh context breaks the frame.

**Spec source:** the originating adversarial-review design bead (the full decision narrative with predicates, alternatives tables with warrant tags, and "what would invalidate" predicates lives on that bead).

## When to invoke

- **Default-on** for any bead carrying own-work content (`--design`, `--acceptance`, plan in `--notes`, decomposition tree) about to be marked `verdict:pass` by its author. Per , *never review your own contract, plan, or decomposition tree* — the rule is legible (this skill exists) and unignorable (orchestrator runs it before stamping), but not mechanically gated.
...
The skill runs an internal loop of up to 3 rounds. One round = one fresh-context dispatch of a reviewer subagent with no shared context.

For each round (1..3):

1. **Dispatch one fresh-context reviewer subagent** with the reviewer brief (see below). No memory of prior rounds — fresh dispatch per round .

 <!-- HARNESS-DISPATCH: role=adversarial-reviewer tier=medium -->

 On **Claude Code**: use `Task(subagent_type="adversarial-reviewer", model="sonnet")` — spawns a fresh subagent with no inherited context.
 On **pi**: use the `subagent` tool with `role="adversarial-reviewer"` and `modelTier="medium"` — spawns a fresh pi subprocess with the adversarial-reviewer role brief.
...
This instruction keeps the orchestrator's dispatch brief and the agent's review structure coherent. The adversarial-reviewer agent file is the single source of truth for the six-criterion order; the SKILL.md carries only the brief-text the orchestrator passes in.
...
**Dispatched:** <date> — one fresh-context reviewer subagent.
...
The orchestrator runs `bd update --add-label=verdict:pass` with `BEADS_ACTOR=reviewer:fresh-subagent`. The actual fresh-context dispatch happens in the prior dispatch call inside this skill's loop; substrate cannot distinguish a forged actor token from a real fresh-context dispatch. The recipe (this skill file) enforces the norm. Per the reviewer-identity design bead D2.

**Substrate identity-check limitation** (per the reviewer-identity design bead D3): the close-time check compares verdict-add actor against the bead's create-event actor. For most beads the inequality holds trivially, but it does NOT verify "reviewer subagent ≠ artifact-author" — only "reviewer subagent ≠ bead-create-author." The discipline that a real fresh dispatch happened is carried by this skill's contract, not by substrate enforcement.
...
- `bd show <bead-id> --json` — read bead's current state for the brief. **This (or `bd show <id>`) is the authoritative read; never brief a reviewer off a raw `.beads/issues.jsonl` grep.** That file is a lagging derived export — it is NOT rewritten on `bd update`/`create`/`close`, so it can silently hand a fresh-context reviewer a *stale, pre-revision* copy of the bead. This exact failure wasted two consecutive review rounds in a prior session: the reviewer reported "no harness target / no canonical_refs" for a design `bd show` showed as fully populated. Always pass `bd show` output into the brief (or instruct the subagent to run `bd show` itself). If a file reader genuinely needs the current export, flush it first: `bd export --all -o .beads/issues.jsonl`.
