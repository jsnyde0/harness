...
6. **Materialize-after-review** — only after `/adversarial-review` returns convergence:
 ...
 - `bd dep add <integration-test-bead> <each-sibling>` for the integration-test bead (parent-first carry-forward) if N ≥ 2 children whose work cross-references.
...
7. **Post-materialize graph verification** (FIRM carry-forward) — run these commands:
...
**Why `/decompose` invokes `/harness compose` rather than inlining** — inlining principles would duplicate logic and fork maintenance; invoking the skill means harness improvements automatically reach all authoring primitives. This is the default; deviate only with reason. `/harness compose` is the soldier-proofed canonical surface (2026-04-23). The same mandate applies to `/brainstorm` at convergence; as of 2026-06-01 `/brainstorm/SKILL.md` still inlines the predicates — that parallel-gap fix is named in commit `d5c40ae`'s body as a separate substrate change, not addressed here.
...
**Canonical refs:** the subcommand-invocation mandate + conjunction-coverage clause; the children-jointly-cover-parent rule.
...
3. **Coverage map present, formatted, in-brief** (FIRM carry-forward).
4. **Parent-first hierarchy** — each child uses `bd create --parent=<parent-id>`; no orphan beads (FIRM carry-forward).
5. **Integration-test bead for multi-child trees** — if N ≥ 2 children cross-reference, a final child of `type=task` or `type=test` depending on all siblings carries the cross-component verification (FIRM carry-forward).
6. **File-conflict detection** — no two children claim ownership of the same file/region (FIRM carry-forward). Orchestrator's draft must include a `## File-claim map` section; reviewer flags any overlap.
7. **Leaf path first-class** (FIRM carry-forward) — "no-split-needed" is a valid outcome with its own audit residue.
8. **Per-child `--design` non-empty when child carries non-trivial decision** — trivial children may have empty `--design`; non-trivial must record the four predicates.
...
10. **Harness conjunction-coverage probe + per-child harness target present in their persistence payloads** — graded on the literal text destined for substrate, not on transcript narration. Required:
 - (a) Each child's draft `--design` payload (the literal text that will be passed to `bd create --design=...` at step 6) contains a `## Harness target` block with all four predicates populated (Signal / Expected green / Rationale / Invalidation); named-skip-with-rationale satisfies presence for trivial children.
