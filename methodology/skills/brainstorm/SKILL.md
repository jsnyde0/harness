...
2. Compose the **`--design`** narrative. Structure:
 ...
 - `## Decisions` — each decision with firmness (FIRM / FLEXIBLE / EXPLORATORY), rationale, alternatives table with warrant-tagged rejection reasons (`direct:` source-evidence quote / `reasoned:` causal argument / `external:` cited source), "what would invalidate this"
 - `## Acceptance contract` — observable / falsifiable end-state
 - **`## Harness target`** — goal-faithful falsifiable observable, authored before `bd create` (mandatory; see below)
 - `## canonical_refs` — every referenced ADR and external spec by full path; if none, write `none — self-contained`
3. Compose the **`--acceptance`** — concise observable contract, restated from the design (Given/When/Then or input→output)
4. **Author `## Harness target` before `bd create`** — mandatory. Compose a goal-faithful falsifiable observable using the four-field predicate structure:
 ...
 - **Invalidation** — what would invalidate this harness choice, signal-shaped (no numeric thresholds).
...
`/adr-write` owns the alignment-question discipline, overlap detection vs existing ADRs, evolution-in-place, and firmness routing. Brainstorm hands the decision over with its rationale + warrant; the alignment question fires in `/adr-write`, not here.
...
- **Skipping the `## canonical_refs` section.** This is a hard rule: mandatory on `--design`. If no ADRs are referenced, write `none — self-contained`; do not omit the section.
...
- substrate-thick / process-thin discipline; output produces substrate (bead), composition is the orchestrator's call.
- bead `--design` predicates (firmness, rationale, alternatives, invalidation) and the mandatory `## canonical_refs` section.
- ADRs evolve in place; relevant to delegation to `/adr-write`.
