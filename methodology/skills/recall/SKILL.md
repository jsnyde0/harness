...
2. **Tier-route by gravitas** (per CLAUDE.md "Substrate orientation"; the three-layer architecture):
...
**Probe for absences, not just presences** (same discipline as `/compound`). A non-probing recall mistakes empty-substrate for "system healthy." The brain must explicitly ask: *What is expected but missing? What pattern would I expect to see recorded that isn't?*
...
- **Skipping the absences probe.** Empty substrate can mean "system healthy" OR "system never written to" — distinguish between these before concluding the substrate is clean.
