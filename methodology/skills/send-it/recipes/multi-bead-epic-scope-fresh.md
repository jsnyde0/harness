...
2. **`/scope-check`** on the proposed epic — outward-look neighbor enumeration (files, sibling beads, ADRs). Default-on for multi-bead-epic authoring .
...
5. **Walk children:**
 - Read `## File-claim map`. Disjoint claims → parallel dispatch (single message, multiple implementer dispatches). Overlapping or no map → serial.
...
6. **Parent re-verify** when all children close: run the parent's `--acceptance` + ADR invalidation checks against the cumulative diff . Any FAIL → file discovered-from child; re-walk.
7. **`/compound`** — default-on at epic close . Produces Record / Promote / Retire candidates; orchestrator folds.

## Watch-outs

- **Skipping `/scope-check`.** Overlap-blindness is the #1 multi-bead failure mode (skeptic punch in context). Default-on for this shape.
- **Walking children without reading the file-claim map.** Parallel dispatch without disjointness verification is the race-condition failure mode is designed to prevent.
- **Skipping parent re-verify.** Children all returning "done" doesn't mean the parent's acceptance holds — that's the infinite-handoff failure mode LangGraph documented .
