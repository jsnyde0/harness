...
### Default-on — epic close (ADR-013 D6)

`bd close` on a parent bead where **both** hold:
- (a) `bd show <id> --json | jq -r '.[0].type == "epic"'` returns `true` *(substrate-legible per ADR-013 D6 sharpening, 2026-05-13; the earlier-drafted `epic` label convention was retired before adoption — `type=epic` is set at bead creation, no duplicate signal)*.
...
1. **Dispatch one fresh-context subagent via `Task()`** with the brief (see below). **No internal rounds; single dispatch.** The originating session carries the work's blind spots — one fresh pass, one output. Per ADR-007 D3 and ADR-012 D3 bucket (a).
...
4. **Orchestrator folds per-entry** (only after audit-log stamp):
   ...
   - **Record → harness.md** → propose inventory update: file path + section + diff sketch (per ADR-013 D6 harness-routing branch); orchestrator applies via `Edit` on `.claude/harness.md` after review
   - **Promote** → invoke `/adr-write` as a separate composition step (per ADR-013 D5 cross-decision reconciliation)
...
**Note on the prior interim discipline.** ADR-013 D5 named an interim where the orchestrator carried L3 authorship inline (read target / apply firmness rule / surface alignment question) because `/adr-write` was unbuilt. `/adr-write` now exists (closed 2026-05-13); the interim is retired. Promote entries hand off via the chain in the "Composition with `/adr-write`" section below — no recipe-level fallback applies on new handoffs.
...
- [edit:<bullet-id> | new] <one-line summary> — <rationale: why this warrants L2A, L2B, or harness.md>
  ...
  scope: global | workspace                                            # L2A only — per ADR-013 D7; default global, workspace when project-specific
...
**Record covers both edit-existing and create-new** — the subagent flags `edit:<bullet-id>` or `new` per entry (ADR-013 D5 collapse of fold-in-place vs propose-L2A). Orchestrator chooses mutate-vs-create at fold-time based on whether an existing bullet already covers the candidate. This mirrors ADR-011 D1's in-place-first preference at the L2A layer.

**Record routing decision tree — three-way fork (ADR-013 D6 harness-routing branch, added 2026-05-22):**
...
If **yes** (primary content is about signal-fit relationships, inventory-cell shaped) → **target: `.claude/harness.md`** inventory-update proposal (file path + section + diff sketch). This is the harness-shaped route per ADR-013 D6 recognition pattern.

If **no** (primary content guides agent behavior, not signal-fit) → proceed to Step 2.

**Step 2 — Decay-behavior test (ADR-013 D1 / D8):** Should this fade if unused?
...
**L2A/L2B routing by kind (ADR-013 D8 — applies inside Step 2 above):**
...
Scope per ADR-013 D7: global by default; workspace when the entry is tied to this project's stack/repo; in-doubt → global (broadens later by scope-narrowing, not re-write).

**Promote handoff caveat.** Targets of kind `[CLAUDE.md ...]` or `[skill ... edit]` are *suggestion-only* downstream per ADR-008 D6 (instruction files are human-scope); `/adr-write` will surface them as one-liners, not apply autonomously. Only `[ADR-NNN edit]` and `[new ADR]` targets land non-interactively after the alignment question. `[new ADR]` targets additionally trigger ADR-008 D7's 5-dim overlap detection inside `/adr-write` before authoring.

**Promote carries countermand reasoning** — the "why is this still right after challenge?" sketch per ADR-013 D5. Load-bearing candidates only; recurrence and count are inputs to judgment, not gates.
...
- **`cass --since <window>`** over recent session JSONL — L1 raw recurrence signals. Prefer 2-term lexical queries with `--explain`; never trust hit counts alone; cross-check against ADRs / git / bd. CASS FTS brittle on short tokens (known calibration entry).
...
**Probe for absences, not just presences** (ADR-013 §Foreseeable failure modes). A non-probing subagent mistakes empty-substrate for "system healthy." The brief must explicitly name: *What is expected but missing? What has never been recorded? What pattern has grown but not been named?* The smoke-test example — missing dogfood ledger — was a structural absence no threshold-based count could surface.
...
## Composition with `/adr-write` (ADR-013 D5)

The chain — quoted explicitly:

> `/compound` Promote → orchestrator → `/adr-write` → alignment question → in-place edit per firmness (ADR-013 D3)

`/compound` proposes; `/adr-write` authors. `/compound` does not author L3 itself — not even on a "quick" L3 candidate. The orchestrator decides which Promote entries to act on and invokes `/adr-write` per entry as a separate composition step. `/adr-write` owns the alignment-question discipline and ADR-011 D1 in-place rules.
...
- Not algorithm-prescribing — no step sequence, no probe enum, no thresholds. The skill lists substrate signals; the subagent composes by judgment. Substrate-thick, process-thin per ADR-012 D1.
- Not a cron job — `/compound` is invoked, not scheduled; default-on at epic close, judgment-routed otherwise.

## Working substrate

- `bd close <id>` / `bd show <id> --json | jq -r '.[0].type == "epic"'` — epic-close trigger evaluation (type-based per ADR-013 D6 sharpened 2026-05-13)
- `bd dep tree <id>` — confirm closing bead has closed children (trigger check)
- `Task()` — single fresh-context dispatch
...
- `Edit .claude/harness.md §<section>` — fold Record → harness.md (harness-shaped lessons per ADR-013 D6 recognition pattern)
...
- **ADR-013 D5** — `/compound` primitive shape; three-section output; Record edit-vs-new collapse; Promote handoff chain. Primary design source.
- **ADR-013 D6** — invocation surface; default-on trigger; substrate residue; substrate signals listed (not prescribed); harness-shaped lesson routing branch + recognition pattern (added 2026-05-22).
- **ADR-013 D7** — global vs local scope conventions for Record entries.
- **ADR-013 D8** — L2A `kind` enum (observation / calibration / procedural-lesson / anti-pattern).
- **ADR-012 D3** — primitive shape pattern; `/compound` registered as primitive #7; bucket (a) fresh-dispatch.
- **ADR-007 D3** — fresh-`Task()` per dispatch; no reuse across invocations.
- **ADR-011 D1** — in-place edit preference; mirrored at L2A layer (edit-existing vs create-new).
- **ADR-006 D10** — warrant tag convention referenced by Promote entries routed to `/adr-write`.
